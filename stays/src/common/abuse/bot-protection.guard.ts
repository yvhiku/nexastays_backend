...
/**
 * Lightweight bot / script client filter for public scrape-prone routes.
 * Enforced in production (or when ENFORCE_BOT_PROTECTION=true).
 * Set ALLOW_SCRIPT_CLIENTS=true to bypass (e.g. trusted server-side crawlers).
 */
@Injectable()
export class BotProtectionGuard implements CanActivate {
  private static readonly BLOCKED_UA =
    /^(curl\/|wget\/|python-requests\/|python-urllib\/|scrapy\/|go-http-client\/|java\/|libwww-perl\/|apache-httpclient\/|okhttp\/|httpie\/|aiohttp\/|node-fetch\/|undici)/i;
