...
Date: 2026-09-07  
Actors exercised: guest/consumer, host A, host B (DEMO OTP consumers), admin (staff login). Suspended **staff** covered in RolesGuard; suspended **consumer** JWT path not fully gated (see 047).
...
| 047 | Suspended users cannot perform restricted actions | **UNVERIFIED** | Proven: staff `FROZEN` denied; `SUSPENDED`/freeze bumps `authz_version` so stale admin JWT denied; host `listing_frozen` blocks listing writes. **Not proven:** consumer `SUSPENDED` does not invalidate or block ordinary JWT access in Identity `JwtStrategy` / Stays `IdentityJwtStrategy` (status not checked on consumer routes). Shared dogfood DB was not mutated to suspend a live consumer. |
...
See [jest.txt](./jest.txt) — Stays 144 + Identity 14 = **158** related tests passed.
