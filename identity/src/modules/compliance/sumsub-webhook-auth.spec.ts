...
describe('Sumsub webhook raw-body authentication', () => {
  ...
  it.each([
    ...
    'accepts a valid %s digest over the exact raw bytes',
    async (header, alg) => {
      const digest = createHmac(alg, secret).update(raw).digest('hex');
      await expect(
        service.processSumsubWebhook(payload, raw, digest, header),
  ...
  it('rejects a modified body even when the original digest is valid', async () => {
    ...
    await expect(
      service.processSumsubWebhook(payload, Buffer.from('{}'), digest),
    ).rejects.toBeInstanceOf(BadRequestException);
  });

  it.each([undefined, '', '00', 'not-a-digest'])(
    'rejects missing or malformed digest %s',
    async (digest) => {
      await expect(
        service.processSumsubWebhook(payload, raw, digest),
      ).rejects.toBeInstanceOf(BadRequestException);
    },
  );
  ...
  it('rejects a request when no dedicated webhook secret is configured', async () => {
    ...
    await expect(
      service.processSumsubWebhook(payload, raw, '00'),
    ).rejects.toBeInstanceOf(BadRequestException);
  });

  it('rejects unsupported signature algorithms', async () => {
    await expect(
      service.processSumsubWebhook(payload, raw, '00', 'HMAC_MD5_HEX'),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
});
