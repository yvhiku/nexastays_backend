...
export class StaysController {
  ...
  async submitHostOnboarding(
    @CurrentUser() user: { userId: string; account_type?: string },
    @Body() body: SubmitHostOnboardingDto,
  ) {
    const source = body.source ?? 'UNKNOWN';
    const submittedFrom = body.submitted_from ?? 'API_HOST_ONBOARDING';
    return this.hostOnboardingService.submitHostOnboarding(user.userId, body, {
      source,
      submitted_from: submittedFrom,
      requireConsumer: user.account_type === 'CONSUMER',
  ...
  async getHostMe(@CurrentUser() user: { userId: string }) {
    return this.hostOnboardingService.getHostMe(user.userId);
  }
  ...
  async submitHostApplication(
    ...
    return this.hostApplicationsService.submit(user.userId, body);
  }
  ...
  async getHostVerification(@CurrentUser() user: { userId: string }) {
    return this.hostsService.getHostVerificationStatus(user.userId);
  }
  ...
  async submitHostVerification(
    ...
    return this.hostsService.submitHostVerification(user.userId, body);
  }
