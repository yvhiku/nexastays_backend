...
mkdir -p "$backup_dir"
cp "$SHARED" "$IDENTITY" "$STAYS" "$backup_dir/"
...
release_tag="$(git -C /opt/nexa/nexastays_backend rev-parse --short=12 HEAD)"
...
set_key "$SHARED" INTERNAL_SERVICE_KEY "$(random_hex 32)"
...
chmod 600 "$SHARED" "$IDENTITY" "$STAYS" "$DB_DIR/.env.db"
